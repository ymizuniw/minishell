...
int	exec_path(void)
{
	set_env(env_list, external_command) // add external_command to env_list,
		as follows : "_=absolute_path_of_command".execve();
	below here, execve() failed,
		then remove previously set external_command from env_list.unset_env(env_list,
		external_command)
}
...
int	search_and_exec(char *cmd, char **env)
{
	...
	*/
	...
	{
		err = 0;
		// the result of search_hash and find_path should be kept for return value,
		// maybe.
...
/ bin / bash(assuming that Bash has been installed in / bin),
	...
	even if it is executed under another shell.It’s a common idiom to use env to find bash even if it’s been installed in another directory : #!
