...
/*   Updated: 2025/11/15 20:31:52 by ymizuniw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	get_value_len(char *value)
{
	if (value == NULL)
		return (0);
...
static int	handle_literal_word(t_word *word, t_gen_word *gw, char *value,
		size_t *i)
{
	doller_literal_wrapper(gw, value, get_value_len(value), i);
...
t_word	*loop_wrapper(char *value, size_t *i)
{
	...
	else if (handle_literal_word(word, &gw, value, i) < 0)
		return (xfree(word), NULL);
	return (word);
}
