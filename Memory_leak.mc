...
部分的に改善したが、clear 使用後にSIGINTを送信しても反応がなく、依然としてプロンプトの連続表示が起きた。
挙動から予測される可能性としては、子プロセスから戻って来た直後にSIGINTシグナルを処理していないため、改行が表示されていない。
根拠として、SIGSTOPは作動していないことが挙げられる。signal(SIGINT, SIG_IGN)の残効ではないか。