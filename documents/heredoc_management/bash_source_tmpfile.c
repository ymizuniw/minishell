...
static char	*get_tmpdir(int flags)
{
	char	*tdir;
...
static int	tmpunlink(const char *fn)
{
	int	r;
...
char	*sh_mktmpname(const char *nameroot, int flags)
{
	const char	*lroot;
	struct stat	sb;
	static int	seeded = 0;
...
int	sh_mktmpfd(const char *nameroot, int flags, char **namep)
{
	const char	*lroot;
	int			e;
		unsigned long x;
...
# endif
		...
		if (tmpnamelen > 0 && tmpnamelen < 32) /* XXX */
			filename[tdlen + 1 + tmpnamelen] = '\0';
		fd = open(filename,
				BASEOPENFLAGS | ((flags & MT_READWRITE) ? O_RDWR : O_WRONLY),
				0600);
...
FILE	*sh_mktmpfp(const char *nameroot, int flags, char **namep)
{
	int		fd;
	FILE	*fp;
...
char	*sh_mktmpdir(const char *nameroot, int flags)
{
	char		*filename;
	int			fd;
	const char	*lroot;
